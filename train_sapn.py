...
from model_span_adversial import ASTE_SPAN
...
def calcuate_polarity(features, polarity_pred, s_p_preds=None):
    ...
    for i in range(size):
        ...
        for j, s in enumerate(polaryity[i][:5]):
            y_true.append(polaryity_label[s])
            y_pred.append(polarity_pred[i][j].item())
            if polaryity_label[s] == polarity_pred[i][j].item():
                hit += 1

            # size = min(5,len(polaryity[i]))
            # if s_i<=2 and len(set(polarity_pred[i][:size].tolist())) != 1:
            #     print("句子情感", s_i)
            #     print("预测值",polarity_pred[i])
            #     print("标注值",polaryity[i])
    ...
    with torch.no_grad():
        ...
        for i in range(n_train):
            ...
            if len(spans_pred[0])==0:
                ...
                oppiton.extend(oppiton_i[1])
                te_pred.extend(te_pred_i[1])
            oppiton_t.extend(oe_split)

        # f1 = cacluate_ote(ote_pred,te_pred,features_test["spans"],features_test["polaryity"])
        # f1 = cacluate_ote_te(ote_te_pred,features_test["spans"],features_test["polaryity"])
        f1 = cacluate_ote_te_2(spans_preds, te_pred, oppiton,
                               features_test["spans"],features_test["polaryity"],features_test["oe"])

        if f1 > best_f1:
            best_f1 = f1
            torch.save(model.state_dict(), args.sentiment_model_path)
...
parse.add_argument("--sentiment_model_path", default="./model/best_model_14res.pt", type=str)
...
parse.add_argument("--vocab", default="./vocab", type=str)
...
if __name__ == '__main__':
    ...
    if not os.path.exists(args.vocab):
        vocab = build_vocab([args.train_file,args.dev_file, args.test_file], "mpqa_full.txt")
        embedding_mat = vocab.load_word_embedding(embedding_file="../../glove.840B.300d.txt")
        vocab.save(args.vocab)
    else:
        vocab = Vocab.load(args.vocab)
        embedding_mat = vocab.get_word_embedding()

    # (data_helper, features_train), (data_helper_dev, features_dev) = build_data_helper(
